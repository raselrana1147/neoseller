$(function ($) {
  ...
  $(document).ready(function () {
    ...
    //  USER shop FORM SUBMIT SECTION

    $(document).on('submit', '#usershop', function (e) {
      e.preventDefault();
      ...
      $.ajax({
        ...
        success: function (data) {
          if ((data.errors)) {
            ...
            $('#usershop input[type=text], #userform input[type=email], #userform textarea').eq(0).focus();
          } else {
            $('.alert-danger').hide();
            $('.alert-success').show();
            $('.alert-success p').html(data);
            $('#usershop input[type=text]').eq(0).focus();
          }
          $('.gocover').hide();
    ...
    // USER shop FORM SUBMIT SECTION ENDS
    ...
    $(document).on('click', '.add-to-wish', function () {
     ...
     $(document).on('click', '#wish-btn', function () {
      ...
      // My shop section
    $(document).on('click', '.add-to-shop', function () {
      $.get($(this).data('href'), function (data) {

        if (data == 1) {
          toastr.success("Successfully added to my shop");

        } else {

          toastr.error("Already added to my shop");
        }
...
// end my shop
    ...
    $(document).on("click", ".adding", function () {
      ...
      var sell_price = $("#sell_price" + itemid).text();
      if (stck != "") {
