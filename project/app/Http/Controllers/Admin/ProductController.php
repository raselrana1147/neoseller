...
class ProductController extends Controller
{
    public function __construct()
    {
         $this->middleware('auth:admin');
          ...
          $this->middleware('superadmin',['only'=>['allProduct']]);
          $this->middleware('merchant',['only'=>['datatables','index','deactive','type',
            'createPhysical','createDigital','createLicense','status','uploadUpdate','store',
            'import','importSubmit','edit','update','featured','feature','destroy','deactivedatatables']]);
    ...
    public function datatables()
    {
         ...
         ->where('pro_owner','=',Auth::guard('admin')
         ->user()->username)->orderBy('id','desc')->get();
    ...
    { 
         $usernaame=Auth::guard('admin')->user()->username;
         $datas = Product::where(['status'=>0,'pro_owner'=>$usernaame])
         ->orderBy('id','desc')->get();
         ...
         return Datatables::of($datas)
                            ...
                            ->addColumn('action', function(Product $data) {
                                ...
                                <a href="' . route('admin-prod-edit',$data->id) . '"> <i class="fas fa-edit"></i>Edit</a></div>';
                            }) 
                            ->rawColumns(['name', 'status', 'action'])
                            ->toJson(); //--- Returning Json Data To Client Side
    }
    ...
    {
       $datas = Product::all();
       return view('admin.product.adminproduct',compact('datas'));
    }
    ...
    public function createProduct(){
         $cats = Category::all();
        $sign = Currency::where('is_default','=',1)->first();
        return view('admin.product.create.product',compact('cats','sign'));
    }
    ...
    public function store(Request $request)
    {
     ...
     // dd($request->all());
        ...
        $data->special_com=$request->special_com;
        $data->rr_price=$request->price;
        $data->pro_owner=Auth::guard('admin')->user()->username;
        $data->proid="p".rand(1000,9999);
        

        if (!empty($request->olq)) {
            $data->olq=$request->olq;
        }

        //discount calculate;

        if ($request->previous_price > $request->price) {
            $thum_discount      =$request->previous_price-$request->price;
            $discount=($thum_discount*100)/1200;
            $data->thum_discount=number_format($discount, 2, '.', '');
        }
    ...
    public function featuresubmit(Request $request, $id)
    {
        //-- Logic Section
            ...
            if($request->is_discount == "")
            {
                ...
                $input['fresh_price'] = null;              
            } 
            $data->fresh_price=$request->fresh_price; 
