...
class CartController extends Controller
{
    ...
    public function cart()
    {
        ...
        return view('front.cart', compact('products','totalPrice','mainTotal','tx','sell_price')); 
    }
   ...
   public function addtocart($id)
    {
        ...
        ->first(['id','user_id','slug','name','photo','size','size_qty','size_price','color','price','stock','type','file','link','license','license_qty','measure','whole_sell_qty','whole_sell_discount','fresh_price','is_discount','discount_date','rr_price']);
        ...
        if ($prod->is_discount==1) {
            ...
            $today=date('m/d/Y');
            $frashdate=$prod->discount_date;

            $rr_today=strtotime($today);
            $rr_frashdate=strtotime($frashdate);
            if ($rr_frashdate > $rr_today ) {
                $prod->price=$prod->fresh_price;
                $prod->size_price=0;
                $prod->save();

            }else{
                
                $prod->price=$prod->rr_price;
                $prod->save();
   ...
   public function addcart($id)
    ...
    public function addnumcart()
    {
        ...
        $prod = Product::where('id','=',$id)->first(['id','user_id','slug','name','photo','size','size_qty','size_price','color','price','stock','type','file','link','license','license_qty','measure','whole_sell_qty','whole_sell_discount','is_discount','fresh_price','discount_date','rr_price']);
        ...
        $oldCart = Session::has('cart') ? Session::get('cart') : null;
        $cart = new Cart($oldCart);
        $cart->addnum($prod, $prod->id, $qty, $size,$color,$size_qty,$size_price,$size_key,$sell_price);
        if($cart->items[$id.$size]['dp'] == 1)
        {
            return 'digital';
        }
        ...
        //App\Models\Cart::sellprice($cart);

        $cart->totalPrice = 0;
        foreach($cart->items as $data)
        $cart->totalPrice += $data['price'];
         //$cart->totalPrice += $data['sell_price'];         
        Session::put('cart',$cart);
        $data[0] = count($cart->items);   
        return response()->json($data);        
    }  
    ...
    public function addbyone()
    {
        ...
        $cart = new Cart($oldCart);
           ...
           $newcart=json_decode($cart);
           //$sprice=$newcart->sell_price;
           //print_r($newcart) ;

        $cart->adding($prod, $itemid,$size_qty,$size_price,$sell_price);
    ...
    public function coupon()
    {
        ...
        $coupon1 = Coupon::where('code','=',$code)->first();
        ...
        $coupon = Coupon::where('code','=',$code)->first();
            ...
            if ($coupon->minamount >$total) {
                 return response()->json(['status'=>1,'message'=>'You have to shop minimum '.$coupon->minamount.' Tk to apply this coupon']);
            }else{
