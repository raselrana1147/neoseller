...
// **************** ADD TO CART MULTIPLE *******************
    ...
    public function addnum($item, $id, $qty, $size, $color, $size_qty, $size_price, $size_key,$sell_price) {
        $size_cost = 0;
        $storedItem = ['qty' => 0, 'size_key' => 0,'size_qty' =>  $item->size_qty,'size_price' => $item->size_price, 'size' => $item->size, 'color' => $item->color, 'stock' => $item->stock, 'price' => $item->price, 'item' => $item, 'license' => '', 'dp' => '0','sell_price'=>$sell_price];
        if($item->type == 'Physical')
        {
            if ($this->items) {
                if (array_key_exists($id.$size, $this->items)) {
        ...
        if(!empty($sell_price)){
             $storedItem['sell_price'] = $sell_price;
        }
        $item->price += $size_cost;
        if(!empty($item->whole_sell_qty))
        {
        ...
        //$storedItem['price'] = $item->price * $storedItem['qty'];
        $storedItem['price'] = $sell_price * $storedItem['qty'];
...
// **************** ADDING QUANTITY *******************
    ...
    public function adding($item, $id, $size_qty, $size_price,$sell_price) {
        $storedItem = ['qty' => 0, 'size_key' => 0,'size_qty' =>  $item->size_qty,'size_price' => $item->size_price, 'size' => $item->size, 'color' => $item->color, 'stock' => $item->stock, 'price' => $item->price, 'item' => $item, 'license' => '','sell_price'=>$sell_price];
        if ($this->items) {
