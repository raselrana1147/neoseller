      ...
      @include('includes.admin.form-both')  
          <div class="content-area">
            <div class="mr-breadcrumb">
                            <div class="row">
                              <div class="col-lg-12">
                                  <h4 class="heading">{{ __('Commission History') }} <a class="add-btn" href=" javascript:history.back();"><i class="fas fa-arrow-left"></i> {{ __('Back') }}</a></h4>
                                  <ul class="links">
                                    <li>
        ...
        {{-- ===body --}}
              <tbody>
                 ...
                 <tr>
                    <td>{{$account->order->order_number}}</td>
                    <td>{{$account->total_amount}} TK</td>
                    <td>{{$account->total_commission}}</td>
                    <td>
                      
                      @if ($account->collect_amount==1)
                       
                       <div class="action-list">
                           <a href="{{route('add.commission.account',$account->order->id)}}" orderId=>Add to account</a>
                       </div>
                    @else
                       <div class="action-list">
                          <a href="javascript:;" >Money Colleted</a>
                       </div>
                       @endif
