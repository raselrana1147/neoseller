...
@section('content')
        ...
        {{-- ===body --}}
         ...
         @foreach ($commissions as $commission)
               <tr>
                   <td>#{{$loop->index+1}}</td>
                   <td>{{App\Models\Admin::findOrFail($commission->merchant_id)->username}}</td>
                   <td>{{$commission->total}}</td>
                    <td> {{Carbon\Carbon::parse(App\Models\Admin::findOrFail($commission->merchant_id)->created_at)->diffForHumans()}}</td>
