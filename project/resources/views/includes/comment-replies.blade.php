              ...
              <div class="comment-footer">
                <div class="links">
                  <a href="javascript:;" class="comment-link reply mr-2"><i
                      class="fas fa-reply "></i>{{ $langg->lang107 }}</a>
                  @if(count($comment->replies) > 0)
                  <a href="javascript:;" class="comment-link view-reply mr-2"><i
                      class="fas fa-eye "></i>{{ $langg->lang108 }}
                    {{ count($comment->replies) == 1 ? $langg->lang109 : $langg->lang110  }}</a>
                  @endif
                  @if(Auth::guard('web')->user()->id == $comment->user->id)
                  <a href="javascript:;" class="comment-link edit mr-2"><i
                      class="fas fa-edit "></i>{{ $langg->lang111 }}</a>
                  <a href="javascript:;" data-href="{{ route('product.comment.delete',$comment->id) }}"
                    class="comment-link comment-delete mr-2"><i class="fas fa-trash"></i>{{ $langg->lang112 }}</a>
                  @endif
                </div>
              </div>
            </div>
              ...
              <div class="comment-footer">
                <div class="links">
                  ...
                  <a href="javascript:;" data-href="{{ route('product.reply.delete',$reply->id) }}"
                    class="comment-link reply-delete mr-2"><i class="fas fa-trash"></i>{{ $langg->lang112 }}</a>
                  @endif
                </div>
              </div>
            </div>
      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <h3 class="text-center"><a href="javascript:;" data-toggle="modal" data-target="#comment-log-reg"
              class="btn login-btn">{{ $langg->lang101 }}</a> {{ $langg->lang103 }} </h3>
          <br>
        </div>
      </div>
        ...
        <li>
          <div class="single-comment">
            ...
            <div class="right-area">
              ...
              <div class="comment-footer">
                <div class="links">
                  <a href="javascript:;" class="comment-link"><i class="fas fa-eye"></i>{{ $langg->lang108 }}
                    {{ count($comment->replies) == 1 ? $langg->lang109 : $langg->lang110 }}</a>
                </div>
              </div>
            </div>
